...
while [ "$counter" -ge 0 ] ; do
  ...
  if [ "$oldrxbytes" -a "$rxbytes" ] ; then
   echo "RXbytes = $(bytestohumanreadable $(($rxbytes - $oldrxbytes))) "
  else
   echo "Monitoring $interface every $sleep seconds. (RXbyte total = $(bytestohumanreadable $rxbytes))"
  fi
  sleep "$sleep"
done
}
